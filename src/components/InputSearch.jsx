...
const InputSearch = () => {
  ...
  useEffect(() => {
      const resultadosSimulados = [
          'LLantas', 'Rines', 'Accesorios', 'Lujos', 'Luces',
           'Aires', 'Espejos', 'Limpieza', 'Polarizados'
      ];
        ...
        <div className="flex rounded-full bg-lContainer px-2 w-full max-w-[400px] dark:bg-dContainer">
          ...
          {query && (<ul className='absolute top-full h-48 w-40 bg-lContainer p-4 dark:bg-dContainer rounded-xl border-solid overflow-auto'>
                  {results.map((result,index) => (
                      <li className='p-2 text-lNeu dark:text-dTextNeu hover:bg-lBackground dark:hover:bg-dBackground cursor-pointer rounded-2xl' key={index}>{result}</li>
                  ))}
              </ul>)}
        </div>
